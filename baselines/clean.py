...
def TDPO(agent, prev_model, expert_states, expert_actions, greedy=False,steps=100,beta=0.1,reject_from="random",clip_grad=False,noise_level=0.6):
    ...
    for i in range(steps):
            ...
            yw_kl=(reference_chosen_logps.exp()*(reference_chosen_logps-policy_chosen_logps)).sum(-1)
            yl_kl=(reference_rejected_logps.exp()*(reference_rejected_logps-policy_rejected_logps)).sum(-1)
            if TDPO2:
                 values=logits-alpha*(yl_kl-yw_kl.detach())
            else:
                values=logits-(yl_kl-yw_kl)
...
if __name__ == "__main__":
    ...
    for step in range(1,1+int(args.total_steps / args.eval_freq)):
        ...
        elif args.method == 'GPODPO':
            loss, margin, positive_reward, negative_reward = GPODPO(agent, prev_model, expert_obs, expert_act, reject_from=args.reject_from, clip_grad=False, beta=args.beta,noise_level=args.noise_level,steps=args.eval_freq)
        elif args.method == 'TDPO':
            loss, margin, positive_reward, negative_reward = TDPO(agent, prev_model, expert_obs, expert_act, reject_from=args.reject_from, clip_grad=False, beta=args.beta,noise_level=args.noise_level,steps=args.eval_freq)
