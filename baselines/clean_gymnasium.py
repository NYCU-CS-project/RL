...
def evaluate_real_return(policy, env_name, n_episodes, horizon, deterministic, seed=0):
    # Create a vectorized environment
    env = gym.vector.make(env_name, num_envs=n_episodes, max_episode_steps=horizon)
    
    # Set seeds for each environment
    env_seeds = np.arange(seed, seed + n_episodes)
    obs, _ = env.reset(seed=int(env_seeds[0]))
    ...
    for _ in range(horizon):
        ...
        with torch.no_grad():
            action, _ = policy(state, deterministic, with_logprob=False)
        action = action.cpu().numpy()
        
        obs, rewards, terminations, truncations, _ = env.step(action)
        dones = np.logical_or(terminations, truncations)
        
        episode_returns += rewards * ~dones  # Only add rewards for active episodes
        episode_lengths += 1 * ~dones  # Increment lengths for active episodes
...
if __name__ == "__main__":
    ...
    horizon = 1000
    n_episodes = 5
    real_return_det, avg_length_det = evaluate_real_return(agent.ac, args.env_name, n_episodes, horizon, deterministic=True,seed=args.seed)
    real_return_sto, avg_length_sto = evaluate_real_return(agent.ac, args.env_name, n_episodes, horizon, deterministic=False,seed=args.seed)
